library(dplyr); library(ggplot2); library(ggpubr);library(data.table)
...
hist(data$HGHT, main="Distribution of height", xlab="height(cm)",
     breaks = 30, freq=F, col="skyblue", border="red")
...
data2 <- data[EXMD_BZ_YYYY %in% c(2009,2015)]
...
## line plot
table3 <- data[,.(smoker=mean(Q_SMK_YN==3,na.rm=T)),keyby="EXMD_BZ_YYYY"] 
